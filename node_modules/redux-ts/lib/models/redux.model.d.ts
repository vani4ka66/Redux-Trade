...
import { MapDispatchToPropsFunction, MapDispatchToPropsParam, MapStateToProps, MapStateToPropsParam } from 'react-redux';
import { AnyAction, Store as ReduxStore } from 'redux';
export declare type HOC<Pin, Pout> = (c: React.ComponentType<Pin>) => React.ComponentType<Pout>;
export declare type Indexer<T = any> = {
    [key: string]: T;
};
export declare type StateToProps<TState = any> = <T extends Indexer, TOwn>(map: MapStateToProps<T, TOwn, TState>) => MapStateToProps<T, TOwn, TState>;
export declare type DispatchToProps = <T extends Indexer<(...params: any[]) => AnyAction>, TOwn>(map: T | MapDispatchToPropsFunction<T, TOwn>) => MapDispatchToPropsFunction<T, TOwn>;
export interface Store<StoreState> extends ReduxStore<StoreState> {
     ...
     * @memberof StoreBuilder
     */
    connected<TStateProps = {}, TDispatchProps = {}, TOwnProps = {}>(mapStateToProps?: MapStateToPropsParam<TStateProps, TOwnProps, StoreState>, mapDispatchToProps?: MapDispatchToPropsParam<TDispatchProps, TOwnProps>): HOC<TStateProps & TDispatchProps & TOwnProps, Exclude<TOwnProps, TStateProps & TDispatchProps>>;
}
